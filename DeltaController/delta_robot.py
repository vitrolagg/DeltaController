...
class DeltaRobot:
    ...
    def update_pose(self, xyz, as_angles=False):
        ...
        if _phi is not None:
            ...
            # print("Valores atualizados de phi:", [round(val, 2) for val in self.phi])
             # Divisão dos ângulos Phi por 0.225 e impressão dos valores
            # divided_phi = [round(abs(val / 0.225), 2) for val in self.phi]
            # print("Valores atualizados de phi divididos por 0.225:", divided_phi)
    ...
    def draw(self):
        ...
        self.draw_trace()
        d = max(self.base_radius, self.passive_arm) * 1.4
