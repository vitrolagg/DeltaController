...
class InverseKinematics:
    ...
    def inverse(self, desired_loc_xyz:np.array):
        ...
        if all(np.abs(beta_rad) <= 1):
            ...
            print('Algo está errado com as configurações ou a pose está fora de alcance')
            return None
...
class ForwardKinematics:
    ...
    @property
    def r(self):
        return np.vstack([self.base_triangle * np.cos(self.theta), self.base_triangle * np.sin(self.theta),
                          np.zeros([3])]).T

    @property
    def R(self):
        return np.vstack([self.gripper_triangle * np.cos(self.theta), self.gripper_triangle * np.sin(self.theta),
                          np.zeros([3])]).T

    def forward(self, phi):
        A = self.rod_active * np.ones([3])
        B = np.sqrt(self.rod_passive**2 - self.rod_active**2)
        alpha = -phi
        beta = np.arccos((A**2 + B**2 - self.rod_passive**2) / (2 * A * B))
        Ct = B * np.sin(beta)
        Cr = B * np.cos(beta)
        Cz = np.zeros([3])
        R = np.vstack([Cr * np.cos(self.theta) - Ct * np.sin(self.theta),
                       Cr * np.sin(self.theta) + Ct * np.cos(self.theta),
                       Cz]).T
        r = np.vstack([self.base_triangle * np.cos(self.theta), self.base_triangle * np.sin(self.theta),
                       np.zeros([3])]).T
        return R - r
